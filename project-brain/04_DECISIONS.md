...
### ADR-028 — R-1: pipeline de recordatorios multi-tenant sobre una única arquitectura (Vercel Cron + Next)

> ADR-027 permanece reservado para "La Landing como representación oficial del estado del producto" (se emite al cierre de LAND-2).

**Context**: `vercel.json` declaraba crons hacia rutas que **jamás existieron en ningún commit** (`git log --all -- app/api/cron` = vacío) — configuración aspiracional introducida en `c496a24` (2025-11-30) cuyo endpoint nunca se escribió; Vercel invocaba 404s a diario. El pipeline eran cuatro piezas inconexas: encolado real (`app/api/bookings/public`), un procesador Deno completo pero nunca desplegado (`supabase/functions/process-notification-queue`, con `@ts-nocheck` — invisible para el type-check), un stub de envío que solo hace `console.log` (`app/api/notifications/send`, con Resend/Twilio en comentarios) y **ningún generador de recordatorios en ninguna parte**. La Landing promete "Recordatorios de citas automáticos"; en producción era falso.

**Decision** (usuario, 2026-07-10 — Acceptance Contract R-1 v2):

1. **Una única arquitectura: todo en Vercel/Next.** El cron `GET /api/cron/send-reminders` (Bearer `CRON_SECRET`, 401 sin él) genera y procesa en el mismo tick. La edge function Deno se **eliminó del repo** (su lógica de envío se portó 1:1 a TypeScript del pipeline de CI); el cron muerto `cleanup-old-appointments` se eliminó de `vercel.json` (si esa limpieza hace falta, nace en el backlog [Operations] con su propio contrato). Regla derivada: **ninguna entrada de `vercel.json` puede referenciar rutas inexistentes en el repo**.
2. **Multi-tenant desde el día uno, sin sobreingeniería** (pregunta filtro: "¿nos acerca a instalar la primera barbería real sin obligar a reescribir cuando exista el modelo multi-tenant?"). La infraestructura habla **solo de Tenant** — nunca de barberías; la especialización vive en la capa de producto. `lib/tenants.ts` es el seam: `getTenants()` hoy devuelve un único tenant armado desde `business_settings` (id estable `DEFAULT_TENANT_ID`); cuando exista la tabla `tenants` (M1) solo cambia esa función. `notification_queue` gana `tenant_id` (patrón latente de [ADR-016](04_DECISIONS.md), pero acá **escrito desde el día uno** para atribución/metering).
3. **Timezone-aware con cualquier IANA**: las citas guardan hora de pared del tenant (DATE+TIME sin zona, script 01), así que nunca se convierten a UTC — se convierte "ahora" a la hora local del tenant vía `Intl` (cero librerías). Seed de referencia: `America/Panama` (UTC-5, sin DST), inyectado por script 36 en el JSON `business`; no está hardcodeado. Cadencia asumida: **Vercel Hobby (1 tick/día, `0 14 * * *` = 9:00 Panamá)**; la lógica es de **ventana "due"** (`cita − offset ≤ ahora < cita`), independiente de la cadencia — pasar a tick horario (Pro) es una decisión operativa, no un cambio de arquitectura.
4. **Dos responsabilidades separadas por diseño, un solo endpoint durante la beta**: `ReminderScheduler` (`lib/notifications/scheduler.ts`) descubre qué generar; `NotificationProcessor` (`lib/notifications/processor.ts`) procesa la cola y envía. El día que necesiten escalar por separado se dividen sin romper contratos.
5. **El dedup vive en la base, no en la aplicación**: índice `UNIQUE (appointment_id, reminder_offset_hours)` (script 36; completo, no parcial — PostgREST no infiere índices parciales en `ON CONFLICT`; las filas no-reminder llevan NULLs que nunca colisionan) + upsert con `ignoreDuplicates`. Procesamiento con claim compare-and-swap (`pending → processing` contando filas afectadas) + reclaim de filas atascadas >15 min (`processing_at`). N ejecuciones concurrentes: cero duplicados.
6. **Cuotas desacopladas**: el pipeline **nunca** conoce planes/créditos/suscripciones — solo consulta `canSendNotification(tenantId)` (hoy siempre permite). Billing evolucionará íntegramente detrás de esa interfaz; cada fila lleva `tenant_id` + `sent_at`, así el metering futuro es un `COUNT`.
7. **Configuración por tenant**: `reminders { enabled, offsetsHours, channels }` en `business_settings.notifications` (accessor único `getTenantReminderConfig` con defaults `[24]h`, email+sms; cada offset genera un recordatorio independiente). Sin UI de edición en R-1 (feature freeze) — se configura por SQL.

**Consequences**: `pnpm test` 59/59 (33 nuevos, TDD estricto: tz/ventana due/dedup/claim/envío con fetch inyectado), `type-check` y `lint` limpios (0 warnings nuevos). `scripts/36-notification-queue-tenant.sql` es additivo con rollback documentado; **pendiente de ejecutar en la instancia real** antes del primer tick. Deuda registrada en [07_TECH_DEBT.md](07_TECH_DEBT.md): el stub `/api/notifications/send` sigue vivo con un consumidor real (`reschedule/route.ts:244`) y hoy NO envía nada real. Cierre del milestone: E2E real (cita con teléfono/email del usuario → invocación manual con el secret → recepción verificada → primera ejecución automática en el dashboard de Vercel).

**Status**: Implemented (2026-07-10) — código completo y validado; falta ejecutar script 36 en la instancia real + E2E de punta a punta con participación del usuario.
